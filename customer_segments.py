...
# 
# 
# **提示：** 企业的类型包括 markets (市场)、cafes (咖啡馆、餐厅)、delis (熟食店、快餐店)，wholesale retailers (大宗零售商/超级市场) 及其他。注意不要使用具体企业的名字，比如说在描述一个餐饮业客户时，你不能使用麦当劳。

# **回答:** 样本0属于 大宗零售商/超级市场，各品类采购接近均值，在牛奶，杂货，洗衣用品和熟食高于均值，比较符合大宗超市的购物需求；样本1采购大部分远低于均值，只有冷冻食品接近均值，属于小型经营商家，属于熟食和快餐店的可能较大；样本2采购金额几乎与均值线保持一致走势，除了在生鲜品类，采购相对较多，整体表现更像一个市场类型的客户；
...
# **回答:** 我尝试预测`Detergents_Paper`(洗涤剂纸), 预测的决定系数是 0.76, 这个特征对于区分用户的消费习惯来说不必要，因为基本可以使用其他特征的组合来拟合或趋近这个特征。
...
# **回答:** 从散布矩阵可以看出，特征`Grocery`与`Detergents_Paper`呈现一定的线性相关性，这个结果验证了我尝试预测的特征`Detergents_Paper`的相关性，不难发现数据分布的均值大于总数，特征呈现正偏态分布。
...
# **回答:** 索引为65, 66, 75, 128, 154的特征向量在多余一个特征下被看作是异常的数据点，这些点应该从数据集中移除，因为这些异常点存在使得把这些值考虑进去后结果出现倾斜，但同时删除过多数据，也会导致数据损失度增加，因此只删除在多个特征下被看作是异常的点，而保留其他点。
...
# **回答:** 第一个和第二个主成分总共表示了0.708的方差，前四个总共表示了0.9311的方差; 第一个主成分代表大宗商超的消费行为，因为影响方差的特征主要是日用百货，第二个主成分代表餐饮商家的消费行为，因为影响方差的特征主要是生鲜冷冻和熟食，第三个主成分代表市场型消费行为，因为影响方差的主要特征是生鲜冷冻食材，对牛奶百货需求等需求较少，第四个主成分代表咖啡馆等快餐连锁商家的消费行为，因为影响方差的主要特征是冷冻与熟食
...
# **回答:** K-Means聚类的优点有：1.原理简单，实现方便，收敛速度快，2.聚类效果较优，3.模型的可解释性较强，4.调参只需要簇数k；高斯混合聚类的优点有：1. 软聚类；2. 聚类的形状灵活；基于数据观察，我选用高斯混合聚类，因为，数据分布呈现正态分布
...
# **回答:** 聚类为2时，轮廓系数为0.425, 聚类数为3时，轮廓系数是0.397； 聚类数目为2时能够得到最佳的轮廓系数
...
# **回答:** Segment0 代表了主要客户的类型的中小规模客户，因为采购品类走势与均值基本一致，但都低于均值；Segment1 代表了大宗商超的客户类型，这些客户采购金额高于均值，且在牛奶百货等日用品的采购中占了较大比重

# In[28]:


data.mean().plot()
true_centers.loc['Segment 1'].plot()
...
# **回答：** 不同的客户分类对派送策略的改变的影响通常是不一致的，因此为了客观准确得知派送策略的调整对哪一类客户更具有积极影响，需要针对不同客户群体分别进行A/B测试，通过对比，得知派送策略的调整更适合哪一类客户，最终帮助针对不同类型的客户制定不同的派送策略。
...
# **回答：** 通过使用非监督学习先对数据集进行聚类，给数据增加聚类的工程特征，再进行分类器训练可以得到性能更好的分类模型，这样对新用户的采购金额进行预测能够得到更加准确的结果，方便对客户制定更个性化的采购策略
...
# **回答：** 从上图可以看出样本数据0和2被划分在一个分类，样本1被划分到另一个分类里，与前面预测0和2为市场性等市场性客户，样本1为小型客户基本一致，因此选择比较好；没有；基本一致；
